 ...
 */
public class SolidBox extends A_SolidBody {
    ...
    public double distance(double x, double y) {
        ...
        double dx = abs(result.getX()) - _scaleX * 0.5;
        double dy = abs(result.getY()) - _scaleY * 0.5;
    ...
    public Vector2D closestSurfacePoint(double x, double y) {
        ...
        x = result.getX();
        y = result.getY();
        double dx = abs(x) - _scaleX * 0.5;
        double dy = abs(y) - _scaleY * 0.5;
    ...
    public Vector2D distanceNormal(double nx, double ny, double x, double y) {
        ...
        if (abs(x) - _scaleX * 0.5 > abs(y) - _scaleY * 0.5) {
            nx = nsgn(x);
            ny = 0.0;
        } else {
            nx = 0.0;
